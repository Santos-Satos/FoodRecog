...
class ResNet_LSTM_Linear(nn.Module):
    
    def __init__(self, n_channel, n_output, drop_rate):
        super().__init__()
        
        self.Conv_1 = nn.Conv2d(1, n_channel, kernel_size=(2, 1), stride=(1, 1))
        ...
        self.Pool = nn.MaxPool2d(kernel_size=(1, 13), stride=(1, 1))
        self.lstm = nn.LSTM(input_size = n_channel*8,
                    hidden_size = 512,
                    batch_first = True,
                    bidirectional=True)
        self.dropout = nn.Dropout(drop_rate)
        self.fc1 = nn.Linear(1024, n_output)# bidirectionalだからhidden_size*2
