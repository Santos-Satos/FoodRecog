...
def main(model, batch_size, optimizer, epochs, progress_flag=True, save_flag=False):
    ...
    for epoch in range(epochs):
        ...
        if save_flag == True and test_acc > max_acc:
            # 学習情報の保存
            outfile = './params/state_ResNet_Linear.pth'
...
if __name__ == "__main__":
    ...
    model = ResNet_Linear(n_channel=n_channel, n_output=len(food_class), drop_rate=drop_rate)
    device = SelectDevice()
    ...
    epochs = 100
    batch_size = 8
    lr = 9.125203343522643e-05
    betas = (0.9, 0.999)
    weight_decay = 5.5464670348424754e-08
